...
export function CardGrid({ cards, deckId, listType }: CardGridProps) {
  ...
  if (groupBy === 'role') {
    ...
    const sortedGroups = Object.entries(grouped).sort(
      ([a], [b]) => (roleImportance[b as CardRole] || 0) - (roleImportance[a as CardRole] || 0)
    )
        ...
        {sortedGroups.map(([role, groupCards]) => (
          <div key={role}>
            <h3 className="text-sm font-semibold text-muted-foreground mb-2 capitalize flex items-center gap-2">
              <Badge className={roleColors[role as CardRole]}>{role}</Badge>
              <span>({groupCards.reduce((sum, c) => sum + c.quantity, 0)})</span>
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
              {groupCards.sort((a, b) => a.card.name.localeCompare(b.card.name)).map(card => (
                <CardItem
                  key={card.card.name}
                  card={card}
                  deckId={deckId}
  ...
  return (
    <div className="p-4">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
        {confirmedCards.sort((a, b) => a.card.name.localeCompare(b.card.name)).map(card => (
          <CardItem
            key={card.card.name}
            card={card}
            deckId={deckId}
